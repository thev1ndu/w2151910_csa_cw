...
To persist data across requests, the application uses a singleton DataStore class with a private constructor and a static getInstance() method, ensuring that only one shared instance exists. All resource classes obtain a reference to this same object. Because Apache Tomcat serves requests concurrently across multiple threads, the DataStore must be thread-safe. Instead of using concurrent collections, the implementation uses standard HashMap and ArrayList combined with method-level synchronized control to ensure safe access to shared state.

The `synchronized` keyword enforces mutual exclusion using the JVM’s intrinsic monitor lock, allowing only one thread at a time to execute critical sections. This guarantees atomicity for multi-step operations (such as adding a sensor and updating its associated room), prevents race conditions, and ensures memory visibility between threads. While this approach may reduce throughput under high concurrency due to locking the entire method for one thread at a time, it provides strong consistency and predictable behavior, which is suitable for this application.
...
Yes, the `DELETE /rooms/{roomId}` operation is idempotent. The first successful call removes the room from the `DataStore` and returns `204 No Content`. If the same request is repeated, the room no longer exists, so the method returns `404 Not Found`. Although the response status code differs between the two calls, the server-side state is identical after both the room is absent. According to HTTP Documentation, idempotency requires that "the side-effects of N > 0 identical requests is the same as for a single request," which is exactly what this implementation guarantees. Additionally, if the room still has sensors assigned, the `RoomNotEmptyException` is thrown, returning `409 Conflict` and preventing data orphans.
